...
**Estado actual:** el estado previo `planned_draft_only` avanzo a `inbound_canary_verified`. Gate 6C.1 tiene destino privado verificado, DNS de Email Routing y reglas entrantes activas para `hello@`, `hola@` y `ola@`; una prueba externa confirmo una entrega por alias, cero entregas para `no-reply@` y catch-all deshabilitado. Gate 6C.2B incorporo el dominio remitente, verifico SPF/DKIM/DMARC y recibio un unico canary humano. Gate 6C.3A selecciono e implemento localmente un aviso interno listo para revision. Gate 6C.3B desplego la ruta cerrada, D1 migrada, rate limiter, destino fijo e identidad opaca fuera de Git. Dos intentos individualmente aprobados invocaron una vez al proveedor cada uno, ambos fallaron sin entrega ni reintento y restauraron inmediatamente el kill switch en OFF. `to: undefined` no resolvio el rechazo remoto. El estado vigente es `corrected_single_canary_attempt_failed_no_retry_kill_switch_off`; D1 registra dos `failed`, cero `sent` y cero `reserved`, mientras uso del proveedor y Gmail confirman cero correos. La candidata `explicit_to_null_with_sanitized_provider_failure_codes` queda solo local. Envios, billing, marketing y automatizaciones permanecen OFF.

La secuencia historica conserva los hitos `private_bindings_ready_kill_switch_off`, `missing_explicit_to_field_for_fixed_destination_binding` y `corrected_off_version_verified_negative_probe_passed`; describen estados anteriores y no reemplazan el estado vigente.
