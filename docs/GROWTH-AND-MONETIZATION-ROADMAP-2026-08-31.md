...
**Estado actual:** el estado previo `planned_draft_only` avanzo a `inbound_canary_verified`. Gate 6C.1 tiene destino privado verificado, DNS de Email Routing y reglas entrantes activas para `hello@`, `hola@` y `ola@`; una prueba externa confirmo una entrega por alias, cero entregas para `no-reply@` y catch-all deshabilitado. Gate 6C.2B incorporo el dominio remitente, verifico SPF/DKIM/DMARC y recibio un unico canary humano. Gate 6C.3A selecciono e implemento localmente un aviso interno listo para revision. Gate 6C.3B desplego la ruta cerrada, D1 migrada, rate limiter, destino fijo e identidad opaca fuera de Git. Tres intentos individualmente aprobados invocaron una vez al proveedor cada uno, fallaron sin entrega ni reintento y restauraron inmediatamente el kill switch en OFF. `to: undefined` y `to: null` no resolvieron el rechazo remoto. La candidata local `explicit_to_private_runtime_destination` usa una variable privada del Worker, nunca el request, y conserva el binding restringido al mismo destino. Paso `16/16` pruebas especificas, pero no fue desplegada. D1 conserva tres `failed`, cero `sent` y cero `reserved`. El estado vigente es `third_canary_failed_private_destination_candidate_local_off`; envios, billing, marketing y automatizaciones permanecen OFF.
