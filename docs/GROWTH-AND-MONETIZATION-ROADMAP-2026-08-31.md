...
**Estado actual:** Gate 6C.1 verifico Email Routing y tres aliases entrantes. Gate 6C.2B verifico el dominio remitente, SPF/DKIM/DMARC y un canary humano. Gate 6C.3B cerro el primer aviso interno con exactamente una entrega comprobada y rollback inmediato al kill switch OFF; D1 conserva un `sent`, tres fallos historicos y cero reservas pendientes. Gate 6C.3C preparo localmente el adaptador que deriva ese aviso desde una solicitud privada persistida leyendo solo `id`, `locale` y `state`. El estado vigente es `private_flow_adapter_local_ready_remote_disabled`: la entrega fija esta verificada, pero la integracion privada no fue desplegada y correo general, clientes, marketing, automatizaciones y billing permanecen OFF.
...
- **Gate 6C.3C:** adaptador local metadata-only preparado. Un trigger privado acepta solo UUID, accion fija y aprobacion humana; D1 entrega exclusivamente `id`, `locale` y `state`; el resultado idempotente queda `prepared_not_sent`. No hubo despliegue, migracion ni correo. Estado `private_flow_adapter_local_ready_remote_disabled`.
