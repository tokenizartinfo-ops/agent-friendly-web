...
17. **Gate 6C - correo operativo:** la politica local `planned_draft_only` sigue gobernando borradores; Email Routing, DNS y tres aliases entrantes quedaron verificados bajo `inbound_canary_verified`. La prueba externa confirmo una entrega por alias y cero para `no-reply@`. **Gate 6C.2A** selecciono Cloudflare Email Service y dejo contrato, costos, baseline saneado y preflight local bajo `provider_selected_remote_unconfigured`; dominio emisor, DNS de salida, billing, binding, envio y automatizaciones siguen OFF y requieren dos decisiones remotas separadas.
...
La frontera local de Gate 6C continua implementada como `planned_draft_only`: acepta solo metadata minima saneada, rechaza cuerpos, adjuntos y secretos probables, y nunca envia. El canary entrante de Gate 6C.1 verifico DNS, routing y entregas externas. Gate 6C.2A selecciono Cloudflare Email Service bajo `provider_selected_remote_unconfigured`, pero no configuro dominio emisor, DNS de salida, binding, billing, lectura agentica ni persistencia. La salida requiere dos decisiones remotas separadas. La evidencia vive en `docs/BLOCK-6C-EMAIL-ROUTING-DRAFT-LOCAL-GATE-2026-08-31.md`, `docs/BLOCK-6C1-EMAIL-INBOUND-CANARY-RUNBOOK-2026-09-02.md` y `docs/BLOCK-6C2-EMAIL-OUTBOUND-CANARY-LOCAL-GATE-2026-09-02.md`.
