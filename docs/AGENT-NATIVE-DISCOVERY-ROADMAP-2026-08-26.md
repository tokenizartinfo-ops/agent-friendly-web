...
16. **Gate 6B - captura consentida remota OFF parcial:** preview publico cerrado y ruta candidata de staging separada con hostname exacto, kill switch, rate limiting obligatorio, cuerpo acotado y D1 remota aislada y vacia. El Worker ya esta desplegado con escrituras OFF; Access y Turnstile remotos faltan para cerrar Gate 6B.2. Datos reales y correo siguen deshabilitados.
...
Gate 6A quedo cerrado. El gate inmediato es **terminar 6B.2 remoto OFF** con Access y Turnstile, sin datos reales y sujeto a confirmacion en el momento de modificar permisos o crear credenciales persistentes. La decision completa vive en:
...
El canary privado OFF de Gate 6B ya valida aislamiento, D1 migrada y cierre por defecto sin contactos. Gate 6B.1 implemento y fusiono un Worker privado dedicado con Cloudflare Access firmado, CORS exacto, rate limiting nativo, Turnstile obligatorio y D1 propia. Sus 357 pruebas, build y dry-run estan aprobados. Gate 6B.2 creo el Worker y D1 remotos, aplico las migraciones y comprobo cero filas con escrituras OFF; todavia faltan Access y Turnstile para cerrar la frontera. La primera escritura sintetica pertenece a Gate 6B.3 y requiere aprobacion separada. La especificacion y evidencia viven en `docs/superpowers/specs/2026-08-31-agent-friendly-web-contact-worker-frontier-v1-design.md`, `docs/BLOCK-6B1-CONTACT-WORKER-LOCAL-2026-08-31.md` y `docs/BLOCK-6B2-CONTACT-WORKER-REMOTE-OFF.md`.
