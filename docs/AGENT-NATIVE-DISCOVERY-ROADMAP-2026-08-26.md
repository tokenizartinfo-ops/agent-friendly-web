...
17. **Gate 6C - correo operativo:** la politica local `planned_draft_only` sigue gobernando borradores; Email Routing, DNS y tres aliases entrantes quedaron verificados bajo `inbound_canary_verified`. Gate 6C.2B incorporo el dominio remitente a Cloudflare Email Service, publico seis DNS y verifico un canary humano con SPF, DKIM y DMARC en `pass`; su cierre historico es `human_canary_verified_binding_blocked`. Gate 6C.3A selecciono `internal_review_ready` y preparo una ruta cerrada `at-most-once`, con destino fijo y auditoria `metadata-only`. Gate 6C.3B alcanzo `private_bindings_ready_kill_switch_off` y `authenticated_negative_probe_verified_kill_switch_off`. Tres intentos transaccionales aprobados individualmente fallaron sin entrega ni reintento y restauraron inmediatamente OFF. La candidata `to: null` permanece en la version OFF `8d759339-5caf-4492-bf6a-ff6a2b3f9801`; D1 conserva tres `failed`, cero `sent` y cero `reserved`. El estado vigente es `third_canary_failed_private_destination_candidate_local_off`. La siguiente candidata `explicit_to_private_runtime_destination` paso `16/16` pruebas especificas: el destinatario solo existe como variable privada del Worker y el binding conserva la restriccion al mismo destino. Permanece local, sin cuarto intento y con `delivery_fix_remotely_verified=false`.
...
La frontera local de Gate 6C continua implementada como `planned_draft_only`: acepta solo metadata minima saneada, rechaza cuerpos, adjuntos y secretos probables, y nunca envia. Gate 6C.1 verifico DNS, routing y entregas entrantes. Gate 6C.2B verifico el dominio de salida y exactamente un correo manual mediante REST API, sin dejar capacidad recurrente. Gate 6C.3A selecciono el aviso interno `internal_review_ready` y preparo codigo, migracion y contrato local con destino fijo, reserva previa, `at-most-once` y auditoria `metadata-only`. Gate 6C.3B verifico la frontera remota y provisiono bindings privados. Tres intentos de la ruta transaccional quedaron `failed`, con tres invocaciones acumuladas, cero entregas y cero reintentos; cada uno restauro inmediatamente OFF. `to: undefined` y `to: null` no resolvieron la entrega en el runtime observado. El estado vigente es `third_canary_failed_private_destination_candidate_local_off`; la candidata `explicit_to_private_runtime_destination` paso `16/16` pruebas especificas y conserva el destinatario fuera del request y de Git. Aun no fue desplegada. Billing, lectura agentica y envios siguen bloqueados y `delivery_fix_remotely_verified=false`. La evidencia agrega `docs/BLOCK-6C3B-EMAIL-REVIEW-READY-THIRD-CANARY-ATTEMPT-2026-09-03.md` a la secuencia historica de Gate 6C.

La secuencia historica permanece auditable: `single_canary_attempt_failed_no_retry_kill_switch_off` identifico `missing_explicit_to_field_for_fixed_destination_binding`; `corrected_off_version_verified_negative_probe_passed` desplego la version OFF `7b25f69e-d30e-4ee4-be0e-c2deafed0f3d`; `corrected_single_canary_attempt_failed_no_retry_kill_switch_off` registro el segundo rechazo y preparo `explicit_to_null_with_sanitized_provider_failure_codes`; y `null_candidate_off_version_verified_negative_probe_passed` verifico esa candidata con el kill switch cerrado. Son hitos anteriores y no sustituyen el estado vigente.
