...
17. **Gate 6C - correo operativo:** Cloudflare Email Routing, DNS y tres aliases entrantes quedaron verificados. Gate 6C.2B verifico Cloudflare Email Service, el dominio remitente, SPF, DKIM, DMARC y un canary humano. Gate 6C.3B cerro `internal_review_ready` con exactamente una entrega fija comprobada y rollback inmediato a OFF. Gate 6C.3C preparo localmente un adaptador que exige una solicitud persistida, lee solo `id`, `locale` y `state`, deriva identificadores idempotentes y devuelve `prepared_not_sent`. Gate 6C.3D prepara la captura same-origin completamente sintetica bajo estado `synthetic_contact_canary_local_ready_remote_disabled`; la prueba remota y el correo permanecen OFF.
...
Gate 6C.3C agrega, solo localmente, la union segura con una solicitud privada. La integracion no recibe PII ni contenido: parte de un UUID, verifica en D1 una fila `new`, lee `id`, `locale` y `state`, y prepara el request idempotente sin enviarlo. El despliegue espera un flujo Gate 6B same-origin y una solicitud sintetica. La evidencia vive en `docs/BLOCK-6C3C-PRIVATE-REVIEW-READY-INTEGRATION-LOCAL-2026-09-03.md`.

Gate 6C.3D reconstruye ese flujo Gate 6B dentro del mismo `afw_canary`, sin reutilizar el Worker legacy. El estado local `synthetic_contact_canary_local_ready_remote_disabled` incorpora UI fija sin PII, fixture `.invalid`, Turnstile de prueba validado en servidor, Access, allowlist hash, limite e idempotencia. Todavia no hubo despliegue ni escritura remota; email y captura publica siguen apagados. La evidencia local vive en `docs/BLOCK-6C3D-SYNTHETIC-CONTACT-CANARY-LOCAL-2026-09-03.md`.
