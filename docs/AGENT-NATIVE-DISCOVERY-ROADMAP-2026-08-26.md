...
17. **Gate 6C - correo operativo:** la politica local `planned_draft_only` sigue gobernando borradores; Email Routing, DNS y tres aliases entrantes quedaron verificados bajo `inbound_canary_verified`. Gate 6C.2B incorporo el dominio remitente a Cloudflare Email Service, publico seis DNS y verifico un canary humano con SPF, DKIM y DMARC en `pass`; su cierre historico es `human_canary_verified_binding_blocked`. Gate 6C.3A selecciono `internal_review_ready` y preparo una ruta cerrada `at-most-once`, con destino fijo y auditoria `metadata-only`. Gate 6C.3B alcanzo `private_bindings_ready_kill_switch_off` y `authenticated_negative_probe_verified_kill_switch_off`. Los hitos historicos `single_canary_attempt_failed_no_retry_kill_switch_off`, `missing_explicit_to_field_for_fixed_destination_binding`, `corrected_off_version_verified_negative_probe_passed` sobre la version OFF `7b25f69e-d30e-4ee4-be0e-c2deafed0f3d` y `corrected_single_canary_attempt_failed_no_retry_kill_switch_off` conservan los dos intentos fallidos y sus rollbacks. La candidata `explicit_to_null_with_sanitized_provider_failure_codes` esta desplegada solo en el canary aislado, version `8d759339-5caf-4492-bf6a-ff6a2b3f9801`, deployment `fec166ba-ca50-4134-9ddb-5f1e4976f125`, con flag OFF. La prueba negativa autenticada devolvio `HTTP 404`, `sent=false`, `email_review_ready_unavailable`, sin invocacion ni correo. El estado vigente es `null_candidate_off_version_verified_negative_probe_passed`; D1 conserva dos `failed`, cero `sent` y cero `reserved`, y la entrega corregida permanece sin verificar.
...
La frontera local de Gate 6C continua implementada como `planned_draft_only`: acepta solo metadata minima saneada, rechaza cuerpos, adjuntos y secretos probables, y nunca envia. Gate 6C.1 verifico DNS, routing y entregas entrantes. Gate 6C.2B verifico el dominio de salida y exactamente un correo manual mediante REST API, sin dejar capacidad recurrente. Gate 6C.3A selecciono el aviso interno `internal_review_ready` y preparo codigo, migracion y contrato local con destino fijo, reserva previa, `at-most-once` y auditoria `metadata-only`. Gate 6C.3B verifico la frontera remota y provisiono bindings privados. Dos intentos de la ruta transaccional quedaron `failed`, con dos invocaciones acumuladas, cero entregas y cero reintentos; cada uno restauro inmediatamente OFF. El segundo demostro que `to: undefined` no es solucion remota suficiente. La candidata `explicit_to_null_with_sanitized_provider_failure_codes` mejora compatibilidad y diagnostico sin guardar detalles crudos y quedo desplegada unicamente en `afw_email_review_ready_canary`, version `8d759339-5caf-4492-bf6a-ff6a2b3f9801`, deployment `fec166ba-ca50-4134-9ddb-5f1e4976f125`, con envio OFF. Su prueba negativa autenticada paso sin nuevas filas ni invocaciones. El estado vigente es `null_candidate_off_version_verified_negative_probe_passed`; billing, lectura agentica y envios siguen bloqueados y `delivery_fix_remotely_verified=false`. La evidencia agrega `docs/BLOCK-6C3B-EMAIL-REVIEW-READY-NULL-CANDIDATE-OFF-NEGATIVE-PROBE-2026-09-03.md` a la secuencia historica de Gate 6C.
