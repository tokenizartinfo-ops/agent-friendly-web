...
17. **Gate 6C - correo operativo:** Cloudflare Email Routing, DNS y tres aliases entrantes quedaron verificados. Gate 6C.2B verifico Cloudflare Email Service, el dominio remitente, SPF, DKIM, DMARC y un canary humano. Gate 6C.3B cerro `internal_review_ready` con exactamente una entrega fija comprobada y rollback inmediato a OFF. Gate 6C.3C preparo el adaptador que exige una solicitud persistida, lee solo `id`, `locale` y `state`, deriva identificadores idempotentes y devuelve `prepared_not_sent`. Gate 6C.3D verifico remotamente la union con una unica solicitud sintetica `.invalid`, un consentimiento `requested_plan` y cero correos. Estado `synthetic_contact_canary_verified_kill_switch_off`; datos reales, captura publica y ambos interruptores permanecen OFF.
18. **Gate 6D - ventas y CRM ligero; privacidad privada verificada y cerrada:** Gate 6D.4C verifico el lifecycle sintetico y Gate 6D.4D completo una unica prueba humana con datos propios detras de Access. El recorrido alta -> exportacion -> rectificacion -> retiro -> borrado termino con el registro `erased`, identificadores directos vacios y cero nuevas oportunidades CRM o entregas de email. El Worker volvio a OFF y todos los flags de datos reales, novedades y escritura sintetica permanecen apagados. Gate 6D.4D2 prepara en Draft PR un aviso operativo ES/EN/PT y un contrato machine-readable que separan tratamiento actual de objetivos futuros; continúan como candidatos sin aprobacion juridica, despliegue ni datos de terceros.
...
22. **Gate 6G - coordinacion avanzada:** A2A, OAuth, MCP owner-scoped, skills customizadas y capsulas por proveedor. La oferta de construccion de MCP para clientes queda especificada, pero no desplegada, en `docs/CLIENT-MCP-SERVER-OFFERING-ARCHITECTURE-V1.md`: comienza con tools publicas read-only, incorpora OAuth solo cuando hay contexto privado y reserva Code Mode para APIs extensas que realmente lo justifiquen.
...
La linea comercial **MCP para clientes** permanece no desplegada y separada del MCP publico read-only de AFW. Su madurez propia va de `MCP-D0` (PDR) a `MCP-D5` (operacion comercial), siempre subordinada a identidad, consentimiento, scope, auditoria y rollback. La arquitectura vive en `docs/CLIENT-MCP-SERVER-OFFERING-ARCHITECTURE-V1.md`.
...
- Cloudflare MCP y buenas practicas: `https://developers.cloudflare.com/agents/model-context-protocol/`
- Cloudflare MCP Code Mode: `https://github.com/cloudflare/mcp`
...
La migracion del origen Cloudflare-native se completo el 2026-09-02. El gate activo de infraestructura es **estabilidad productiva y decision de retiro legado**: repetir el smoke publico, confirmar que D1 conserva cero escrituras funcionales no autorizadas, mantener el recibo exacto de rollback y decidir separadamente cuando retirar el binding Sites. La ruta heredada 6B.3 fue reemplazada por el canary same-origin 6C.3D ya verificado.

En el track de privacidad, Gate 6D.4D quedo cerrado el 2026-09-05 despues de una unica prueba humana con datos propios. La evidencia agregada confirma borrado y saneamiento, mientras el switch del piloto y los flags de contacto real, derechos publicos, retencion y novedades volvieron a `false`. Gate 6D.4D2 preparo localmente la politica operativa candidata, sus rutas ES/EN/PT y el contrato de estado. El siguiente gate exige revisar identidad juridica, domicilio, jurisdicciones, responsable de tratamiento, copy, retencion, procesadores, canal de derechos y soporte antes de publicar o aceptar datos de terceros. Email recurrente, CRM operativo, pagos y toda capacidad A2A transaccional siguen diferidos.
...
El mismo 2026-09-02 se corrigio un falso `AF-0` exclusivo del autoescaneo del custom domain: los `fetch()` del Worker hacia su propio origen devolvian `522`. La configuracion `global_fetch_strictly_public` restauro la inspeccion por el front door publico sin cambiar puntuacion ni controles SSRF. Canary y produccion devolvieron `95/100`, el control externo con Tokenizart conservo `23/100` y D1 permanecio sin escrituras. La version productiva de reemplazo es `33ac170d-6b16-4a48-95c7-bbe31d34792a`; por eso la observacion de estabilidad continua abierta y el baseline historico anterior no se reescribe. El recibo vive en `docs/SCANNER-SELF-AUDIT-INCIDENT-2026-09-02.md` y `docs/evidence/scanner-self-audit-fix-2026-09-02.json`.
...
La frontera local de Gate 6C continua implementada como `planned_draft_only`: acepta solo metadata minima saneada, rechaza cuerpos, adjuntos y secretos probables, y nunca envia. Gate 6C.1 verifico DNS, routing y entregas entrantes. Gate 6C.2B verifico el dominio de salida y exactamente un correo manual mediante REST API, sin dejar capacidad recurrente. Gate 6C.3A selecciono el aviso interno `internal_review_ready` y preparo codigo, migracion y contrato local con destino fijo, reserva previa, `at-most-once` y auditoria `metadata-only`. Gate 6C.3B verifico la frontera remota y provisiono bindings privados. Tres intentos de la ruta transaccional quedaron `failed`; `to: undefined` y `to: null` no resolvieron la entrega. La candidata `explicit_to_private_runtime_destination` se desplego primero con flag OFF y despues realizo un unico intento aprobado: D1 registro una fila `sent`, Gmail confirmo la recepcion y no hubo reintento. El rollback restauro OFF inmediatamente. El estado vigente es `fixed_destination_canary_verified_kill_switch_off` y `delivery_fix_remotely_verified=true`. Billing, correos a clientes, lectura agentica y automatizacion siguen bloqueados. La evidencia vigente es `docs/BLOCK-6C3B-EMAIL-REVIEW-READY-VERIFIED-CANARY-2026-09-03.md`.

Gate 6C.3C agrego la union segura con una solicitud privada. La integracion no recibe PII ni contenido: parte de un UUID, verifica en D1 una fila `new`, lee `id`, `locale` y `state`, y prepara el request idempotente sin enviarlo. Gate 6C.3D la desplego y verifico solamente con un fixture sintetico; datos reales y envio siguen bloqueados. Estado `private_flow_adapter_remote_verified_kill_switch_off`.

Gate 6C.3D reconstruyo ese flujo Gate 6B dentro del mismo `afw_canary`, sin reutilizar el Worker legacy. La prueba privada creo exactamente una solicitud sintetica `.invalid` y un consentimiento `requested_plan`, preparo el aviso como `prepared_not_sent` y no altero las cuatro filas historicas de entrega de email. El primer intento Turnstile fallo antes de escribir; se corrigio la semantica de las credenciales oficiales de prueba, manteniendo accion y host estrictos para credenciales reales. El rollback dejo captura sintetica y email en OFF. Estado `synthetic_contact_canary_verified_kill_switch_off`; evidencia en `docs/BLOCK-6C3D-SYNTHETIC-CONTACT-CANARY-REMOTE-2026-09-03.md`.

La secuencia historica permanece auditable: `human_canary_verified_binding_blocked` verifico primero el proveedor sin dejar capacidad recurrente; `private_bindings_ready_kill_switch_off` cerro la frontera transaccional; `single_canary_attempt_failed_no_retry_kill_switch_off` identifico `missing_explicit_to_field_for_fixed_destination_binding`; `corrected_off_version_verified_negative_probe_passed` desplego la version OFF `7b25f69e-d30e-4ee4-be0e-c2deafed0f3d`; `corrected_single_canary_attempt_failed_no_retry_kill_switch_off` registro el segundo rechazo y preparo `explicit_to_null_with_sanitized_provider_failure_codes`; `null_candidate_off_version_verified_negative_probe_passed` verifico la version `8d759339-5caf-4492-bf6a-ff6a2b3f9801` con el kill switch cerrado; y `third_canary_failed_private_destination_candidate_local_off` preparo el destino privado explicito. Son hitos anteriores y no sustituyen el estado vigente.

Gate 6D dispone ademas de un planificador local `local_planning_only` que separa contacto y oportunidad, impide saltos de etapa y no copia emails, cuerpos o credenciales. La restriccion historica que bloqueaba toda D1 remota se satisfizo de forma acotada al cerrar el canary sintetico 6D.4C y, despues, la prueba humana privada 6D.4D. Ambos usaron exclusivamente la D1 canary, con flags publicos OFF y rollback preparado. Eso no autoriza CRM operativo ni datos de terceros. La evidencia base vive en `docs/BLOCK-6D-CRM-LITE-LOCAL-GATE-2026-08-31.md`; el cierre humano privado vive en `docs/BLOCK-6D4D-PRIVATE-HUMAN-PRIVACY-PILOT-2026-09-04.md`.
...
El canary privado OFF de Gate 6B valido aislamiento, Access deny-by-default, Turnstile restringido, D1 migrada y cierre sin contactos. Gate 6B.1 implemento y fusiono un Worker privado dedicado con Cloudflare Access firmado, CORS exacto, rate limiting nativo, Turnstile obligatorio y D1 propia. Sus 357 pruebas, build y dry-run fueron aprobados. Gate 6B.2 creo la frontera remota completa, mantuvo escrituras OFF y comprobo cero filas. Esa superficie legacy fue retirada; la escritura sintetica se realizo despues en el canary canonico same-origin de Gate 6C.3D. La especificacion historica vive en `docs/superpowers/specs/2026-08-31-agent-friendly-web-contact-worker-frontier-v1-design.md`, `docs/BLOCK-6B1-CONTACT-WORKER-LOCAL-2026-08-31.md` y `docs/BLOCK-6B2-CONTACT-WORKER-REMOTE-OFF.md`.
