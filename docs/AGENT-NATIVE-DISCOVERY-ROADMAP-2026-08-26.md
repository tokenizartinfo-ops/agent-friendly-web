...
El canary privado OFF de Gate 6B ya valida aislamiento, D1 migrada y cierre por defecto sin contactos. El siguiente subgate se disena como un Worker privado dedicado con Cloudflare Access firmado, rate limiting nativo, Turnstile y D1 propia; no se aceptaran headers de identidad sin verificacion JWT ni se abrira una escritura antes de una aprobacion separada. La especificacion vive en `docs/superpowers/specs/2026-08-31-agent-friendly-web-contact-worker-frontier-v1-design.md`.
