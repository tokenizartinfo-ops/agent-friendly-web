...
17. **Gate 6C - correo operativo:** la politica local `planned_draft_only` sigue gobernando borradores; Email Routing, DNS y tres aliases entrantes quedaron verificados bajo `inbound_canary_verified`. Gate 6C.2B incorporo el dominio remitente a Cloudflare Email Service, publico seis DNS y verifico un canary humano con SPF, DKIM y DMARC en `pass`; su cierre historico es `human_canary_verified_binding_blocked`. Gate 6C.3A selecciono `internal_review_ready` y preparo localmente una ruta cerrada `at-most-once`, con destino fijo y auditoria `metadata-only`. Gate 6C.3B desplego esa ruta en el canary, aplico la migracion `0006`, configuro rate limiting, destino fijo e identidad opaca fuera de Git y alcanzo `private_bindings_ready_kill_switch_off`. Luego supero una prueba negativa autenticada con D1 en cero y avanzo a `authenticated_negative_probe_verified_kill_switch_off`. El primer intento real confirmado invoco una vez al proveedor pero no envio correo; no hubo reintento y el flag volvio inmediatamente a OFF. El estado vigente es `single_canary_attempt_failed_no_retry_kill_switch_off`; se corrigio localmente `missing_explicit_to_field_for_fixed_destination_binding`, aun sin verificacion remota.
...
La frontera local de Gate 6C continua implementada como `planned_draft_only`: acepta solo metadata minima saneada, rechaza cuerpos, adjuntos y secretos probables, y nunca envia. Gate 6C.1 verifico DNS, routing y entregas entrantes. Gate 6C.2B verifico el dominio de salida y exactamente un correo manual mediante REST API, sin dejar capacidad recurrente. Gate 6C.3A selecciono el aviso interno `internal_review_ready` y preparo codigo, migracion y contrato local con destino fijo, reserva previa, `at-most-once` y auditoria `metadata-only`. Gate 6C.3B verifico la frontera remota, provisiono bindings privados y supero el probe autenticado con `404`, `sent=false` y D1 en cero bajo `authenticated_negative_probe_verified_kill_switch_off`. El unico intento posterior quedo en `failed`, con una invocacion, cero entregas y cero reintentos; el kill switch fue restaurado a OFF. El estado vigente es `single_canary_attempt_failed_no_retry_kill_switch_off`. El diagnostico `missing_explicit_to_field_for_fixed_destination_binding` se corrigio solo localmente; billing, lectura agentica y envios siguen bloqueados. Antes de otro mensaje deben completarse validacion, despliegue OFF, prueba negativa y una nueva confirmacion humana en el momento de la accion. La evidencia vive en `docs/BLOCK-6C-EMAIL-ROUTING-DRAFT-LOCAL-GATE-2026-08-31.md`, `docs/BLOCK-6C1-EMAIL-INBOUND-CANARY-RUNBOOK-2026-09-02.md`, `docs/BLOCK-6C2-EMAIL-OUTBOUND-CANARY-LOCAL-GATE-2026-09-02.md`, `docs/BLOCK-6C2B-EMAIL-OUTBOUND-REMOTE-CANARY-2026-09-02.md`, `docs/BLOCK-6C3A-EMAIL-REVIEW-READY-LOCAL-GATE-2026-09-02.md`, `docs/BLOCK-6C3B-EMAIL-REVIEW-READY-REMOTE-CLOSED-2026-09-02.md`, `docs/BLOCK-6C3B-EMAIL-REVIEW-READY-PRIVATE-BINDINGS-2026-09-02.md`, `docs/BLOCK-6C3B-EMAIL-REVIEW-READY-AUTHENTICATED-NEGATIVE-PROBE-2026-09-02.md` y `docs/BLOCK-6C3B-EMAIL-REVIEW-READY-SINGLE-CANARY-ATTEMPT-2026-09-03.md`.
