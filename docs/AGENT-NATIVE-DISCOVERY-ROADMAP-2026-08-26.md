...
18. **Gate 6D - ventas y CRM ligero, frontera local preparada; privacidad sintetica verificada:** Gate 6D.4C alcanzo `private_synthetic_lifecycle_verified_kill_switch_off` en `afw_canary`. La migracion `0008_contact_privacy_lifecycle.sql` y un lifecycle completo se verificaron sobre el unico fixture fijo `.invalid`; el replay no agrego eventos y el Worker volvio a OFF. Contactos reales, derechos publicos, retencion, novedades, correo, propuestas, pagos y sitios de clientes permanecen deshabilitados. El siguiente gate es `private_human_privacy_pilot_legal_review_required`; el resultado sintetico no implica preparacion para privacidad real ni aprobacion legal.
