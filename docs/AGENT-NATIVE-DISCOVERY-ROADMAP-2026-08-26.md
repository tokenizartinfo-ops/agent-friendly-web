...
17. **Gate 6C - correo operativo:** la politica local `planned_draft_only` sigue gobernando borradores; Email Routing, DNS y tres aliases entrantes quedaron verificados bajo `inbound_canary_verified`. Gate 6C.2B incorporo el dominio remitente a Cloudflare Email Service, publico seis DNS y verifico un canary humano con SPF, DKIM y DMARC en `pass`; su cierre historico es `human_canary_verified_binding_blocked`. Gate 6C.3A selecciono `internal_review_ready` y preparo una ruta cerrada `at-most-once`, con destino fijo y auditoria `metadata-only`. Gate 6C.3B alcanzo `private_bindings_ready_kill_switch_off` y `authenticated_negative_probe_verified_kill_switch_off`. El primer diagnostico fue `missing_explicit_to_field_for_fixed_destination_binding` y la version cerrada `7b25f69e-d30e-4ee4-be0e-c2deafed0f3d` alcanzo luego `corrected_off_version_verified_negative_probe_passed`. Dos intentos individualmente aprobados invocaron una vez al proveedor cada uno, fallaron sin entrega ni reintento y volvieron inmediatamente a OFF. La correccion `to: undefined` no resolvio el rechazo remoto. El estado vigente es `corrected_single_canary_attempt_failed_no_retry_kill_switch_off`; D1 conserva dos `failed`, cero `sent` y cero `reserved`. La candidata `explicit_to_null_with_sanitized_provider_failure_codes` permanece local y sin desplegar.
