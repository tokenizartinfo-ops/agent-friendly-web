...
17. **Gate 6C - correo operativo:** la politica local `planned_draft_only` sigue gobernando borradores; Email Routing, DNS y tres aliases entrantes quedaron verificados bajo `inbound_canary_verified`. Gate 6C.2B incorporo el dominio remitente a Cloudflare Email Service, publico seis DNS y verifico un canary humano con SPF, DKIM y DMARC en `pass`. Gate 6C.3B cerro el aviso interno `internal_review_ready`: tras tres fallos historicos, `explicit_to_private_runtime_destination` uso un secreto privado y un binding restringido al mismo destino. Un cuarto intento individualmente aprobado produjo exactamente una fila `sent` y una recepcion Gmail, sin adjuntos ni reintentos. El rollback restauro inmediatamente OFF. Estado `fixed_destination_canary_verified_kill_switch_off`; `delivery_fix_remotely_verified=true`. El envio general, destinatarios arbitrarios, clientes, marketing y automatizacion recurrente siguen bloqueados.
