...
El baseline del 2026-09-02 quedo verde y es reproducible con `npm run web:audit:stability`: verifica once rutas publicas/privadas, la frontera Access, la vigencia del ledger, el Worker y binding D1 remotos, el custom domain observado por Cloudflare, el rollback observado por Sites y una consulta D1 `SELECT` fija con cero escrituras. La observacion saneada de los planos de control caduca en 24 horas; debe renovarse para cada corrida decisoria. La ventana permanece abierta hasta al menos el `2026-09-09`; entonces se repetira el control y se preparara una decision separada sobre Sites. La evidencia vive en `docs/CLOUDFLARE-NATIVE-STABILITY-BASELINE-2026-09-02.md`, `docs/evidence/cloudflare-native-control-plane-observation.json` y `docs/evidence/cloudflare-native-stability-baseline-2026-09-02.json`.
