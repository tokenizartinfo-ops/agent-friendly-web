...
export function analyzeRobots(body = '') {
  ...
  const hasWildcardGroup = /user-agent\s*:\s*\*/i.test(source);
  const wildcardBlocksAll = hasWildcardGroup && /disallow\s*:\s*\/\s*(?:#.*)?$/im.test(source);
  ...
  return {
    ...
    allowsPublicCrawl: hasWildcardGroup && !wildcardBlocksAll,
  };
}
