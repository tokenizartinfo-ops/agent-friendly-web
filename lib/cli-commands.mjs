...
async function runRegistryGetCommand(parsed, dependencies) {
  ...
  try {
    ...
    if (rawProfile?.contract !== PUBLIC_PROFILE_CONTRACT) {
      throw new Error("El contrato del perfil no coincide con la version publica soportada.");
    }
    if (parsed.version !== undefined && rawProfile.version !== parsed.version) {
      throw new Error("La version devuelta no coincide con la version solicitada.");
    }
