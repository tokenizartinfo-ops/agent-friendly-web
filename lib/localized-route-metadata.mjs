...
import { PUBLIC_FAQ_COPY } from './public-faq.mjs';
...
import { privacyNoticeCopy } from './privacy-notice-copy.mjs';
...
function routeCopy(routeKey, locale) {
  if (routeKey === 'privacy') {
    const privacy = privacyNoticeCopy(locale);
  ...
  if (routeKey === 'faq') return { title: PUBLIC_FAQ_COPY[locale].title, description: PUBLIC_FAQ_COPY[locale].intro };
