...
export function readContactStagingPolicy(input = {}) {
  ...
  return {
    ...
    uiEnabled: input.CONTACT_STAGING_UI_ENABLED === 'true',
...
function authorizeContactStagingIdentity(policy, requestHost, identity = {}) {
...
export function authorizeContactStagingView(policy, requestHost, identity = {}) {
  if (policy?.uiEnabled !== true) {
    return { allowed: false, status: 404, code: 'contact_staging_unavailable' };
  }
  return authorizeContactStagingIdentity(policy, requestHost, identity);
}

export function authorizeContactStaging(policy, requestHost, identity = {}) {
  const authorization = authorizeContactStagingIdentity(policy, requestHost, identity);
  ...
  return {
    ...
    rateLimitKey: `contact:${authorization.actor.userId}`,
  };
}
