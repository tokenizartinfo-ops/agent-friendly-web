...
function transferredBytes(response) {
  if (response.bodyBytes instanceof Uint8Array) return response.bodyBytes;
...
async function fetchResource(url, accept, dependencies) {
  ...
  if (response.truncated === true) {
    throw integrityError(
      `Recurso OKF incompleto por exceder el limite de lectura: ${url.pathname}.`,
      { path: url.pathname },
    );
  }
  return response;
...
export async function verifyRemoteOkf(options, dependencies = {}) {
  ...
  if (checksums.get("manifest.json") !== hash(transferredBytes(manifestResponse))) {
  ...
  await Promise.all(entries.map(async (entry) => {
    ...
    const actual = hash(transferredBytes(response));
