...
const secretPattern = /(api[_ -]?key|password|contrase(?:n|ñ)a|bearer\s+[a-z0-9._-]+|private[_ -]?key|secret[_ -]?key|sk-[a-z0-9_-]{8,}|ghp_[a-z0-9]{8,})/i;
...
export function analyzeIntakeNotes(input = '') {
  const text = cleanText(input, 5000);
  if (!text) {
    return { contract: 'intake-assistant.v1', blocked: false, persistence: 'none', autonomousWrite: false, suggestions: [], warning: 'Agrega una descripcion para preparar propuestas.' };
  }
  ...
  if (secretPattern.test(text)) {
    return {
      ...
      suggestions: [],
      warning: 'El texto parece incluir credenciales o secretos. Retiralos antes de continuar.',
    };
  }
  ...
  const audienceMatch = text.match(/(?:nos encuentren|para|dirigido a|audiencia(?: es|:)?)\s+([^.!?]{3,140})/i);
  ...
  return {
    ...
    warning: 'Revisa cada propuesta. Nada se guarda ni se publica desde este prototipo.',
  };
}
