...
export function matchPublicFaq(message, locale = 'es') {
  ...
  for (const entry of faqEntries(locale)) {
    ...
    const exact = phrases.some((phrase) => phrase.length > 2 && input.includes(phrase));
    const tokens = new Set(phrases.join(' ').split(' ').filter((token) => token.length > 2));
    const overlap = [...inputTokens].filter((token) => tokens.has(token)).length;
    const score = (exact ? 10 : 0) + overlap;
    if (score > bestScore && (exact || overlap >= 2)) {
      best = entry;
      bestScore = score;
    }
  }
