...
export function buildCloudflareNativeStabilityReport({ observedAt, status, smoke, infrastructure, database }) {
  ...
  const checks = [
    ...
    check('cloudflare_access_edge_signed', access?.origin === ORIGIN && access?.protected_route === '/api/projects' && [301, 302, 303, 307, 308].includes(access?.status) && access?.team_domain === 'tokenizart.cloudflareaccess.com' && access?.audience === CLOUDFLARE_ACCESS_AUD && access?.login_path === '/cdn-cgi/access/login/agentfriendlyweb.dev' && access?.redirect_url === '/api/projects' && access?.resource_metadata_url === `${ORIGIN}/.well-known/cloudflare-access-protected-resource/api/projects` && access?.resource_metadata_protected === true && access?.meta_signature_verified === true && typeof access?.signing_key_id === 'string', access?.audience),
