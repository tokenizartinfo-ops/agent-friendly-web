...
function response(
  step,
  lead,
  { already = false, includeExport = false, resumeStage = 0 } = {},
) {
  const result = {
    status: already
  ...
  if (Number.isInteger(resumeStage) && resumeStage >= 1 && resumeStage <= 4) {
...
async function enroll(database, value, keys) {
  ...
  if (lead) {
    ...
    if (currentStage > 1) {
      return response('enroll', lead, { already: true, resumeStage: currentStage });
    }
    if (
      lead.policyVersion === CONTACT_PRIVACY_POLICY_VERSION
      && lead.lastInteractionAt
      && lead.retentionExpiresAt
    ) return response('enroll', lead, { already: true, resumeStage: 1 });
  }
...
function safeHttpResult(result, expectedAction, expectedEmail) {
  ...
  if (Object.hasOwn(value, 'resumeStage')) {
    if (
      expectedAction !== 'enroll'
      || value.status !== 'private_human_privacy_pilot_step_already_completed'
      || !Number.isInteger(value.resumeStage)
      || value.resumeStage < 1
      || value.resumeStage > 4
    ) return null;
  }
