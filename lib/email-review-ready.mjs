...
export function normalizePrivateEmailDestination(value) {
  if (typeof value !== 'string') return null;
  const destination = value.trim();
  if (!destination || destination.length > 254 || destination.includes('..')) return null;

  const separator = destination.indexOf('@');
  if (separator <= 0 || separator !== destination.lastIndexOf('@')) return null;
  const localPart = destination.slice(0, separator);
  ...
  if (domain.length > 253 || labels.length < 2 || labels.some((label) => !EMAIL_DOMAIN_LABEL.test(label))) {
    return null;
  }
  return destination;
}

export function buildEmailReviewReadyMessage(input = {}, privateDestination) {
  const validation = validateEmailReviewReadyRequest(input);
  if (!validation.ok) return validation;

  const destination = normalizePrivateEmailDestination(privateDestination);
  if (!destination) {
    return failure('private_destination_unavailable');
  }
  ...
  return {
    ok: true,
    message: {
      // The value comes only from a private Worker secret; callers cannot supply it.
      to: destination,
