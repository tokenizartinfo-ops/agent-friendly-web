...
function validateConsentInput(input) {
  ...
  const copyValidation = validateConsentCopyVersion(value.purpose, value.copyVersion);
  if (
...
function validatePrivacyInput(input) {
  ...
  try {
    ...
    return {
      ...validation.value,
      idempotencyKey: normalizeUuid(validation.value.idempotencyKey),
    };
  } catch {
    return null;
  }
...
function generatedMetadata(overrides) {
  ...
  try {
    ...
    return { id: normalizeUuid(id), now };
  } catch {
    throw invalidInput();
  }
}
...
function classifyExisting(existing, requestHash) {
  ...
  const id = UUID.test(existing.id) ? normalizeUuid(existing.id) : existing.id;
  return existing.requestHash === requestHash
    ? { id, persisted: true, duplicate: true, conflict: false }
    : { id, persisted: false, duplicate: false, conflict: true };
}
