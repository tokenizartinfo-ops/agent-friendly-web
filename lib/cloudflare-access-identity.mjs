...
export async function verifyCloudflareAccessJwt({
  ...
  try {
    const { payload, protectedHeader } = await jwtVerify(assertion, keySet || remoteKeySet(issuer), {
      issuer,
      audience: expectedAudience,
      algorithms: ['RS256'],
    });
    if (protectedHeader.typ !== 'JWT') return fail();
    const userId = typeof payload.sub === 'string' ? payload.sub.trim() : '';
    const email = normalizeEmail(payload.email);
    if (!userId || userId.length > 200 || !email) return fail();
