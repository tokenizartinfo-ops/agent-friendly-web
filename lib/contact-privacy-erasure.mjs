...
const CONTACT_STATES = new Set(['new', 'erased']);
const RESTRICTION_STATES = new Set(['none', 'restricted']);
...
async function findLead(database, leadId) {
  try {
    return await database
      .prepare(`SELECT id, erased_at AS erasedAt, idempotency_key AS idempotencyKey
...
async function findMatchingSuppressions(database, value, ownerIdempotencyKey = value.idempotencyKey) {
  try {
    const response = await database
      .prepare(`SELECT
        ...
        WHERE (email_hmac = ? AND purpose = ?) OR idempotency_key IN (?, ?)
        ORDER BY id LIMIT 2`)
      .bind(value.emailHmac, value.purpose, value.idempotencyKey, ownerIdempotencyKey)
      .all();
...
function suppressionMatches(row, value, minimumExpiryMs, ownerIdempotencyKey = '') {
  ...
  return Boolean(
    expiresAt
    && (
      normalizeUuid(stored.idempotencyKey) === ownerIdempotencyKey
      || (stored.emailHmac === value.emailHmac && stored.purpose === value.purpose)
    )
...
function suppressionsAreCompatible(
  rows,
  value,
  minimumExpiryMs,
  required,
  ...
  return rows.length === 1
    && suppressionMatches(rows[0], value, minimumExpiryMs, ownerIdempotencyKey);
}
...
async function deriveCommittedPurpose(value, lifecycle) {
  const ownerIdempotencyKey = normalizeUuid(lifecycle.idempotencyKey);
  const matches = [];
  try {
    for (const purpose of PURPOSES) {
      const candidateHash = await sha256(JSON.stringify([
        value.leadId,
        value.emailHmac,
        purpose,
        value.policyVersion,
      ...
      if (candidateHash === lifecycle.requestHash) matches.push(purpose);
    }
  } catch {
    throw erasureFailed();
  }
  if (matches.length !== 1) throw erasureFailed();
  return { ownerIdempotencyKey, purpose: matches[0] };
}
...
async function validateCommittedErasure(
  ...
  if (lead.idempotencyKey !== erasedIdempotency) throw erasureFailed();
  ...
  const sameRequest = normalizeUuid(lifecycle.idempotencyKey) === value.idempotencyKey
    && lifecycle.requestHash === requestHash;
  ...
  const { ownerIdempotencyKey, purpose } = await deriveCommittedPurpose(value, lifecycle);
  const winnerValue = { ...value, purpose };
  const suppressions = await findMatchingSuppressions(
    database,
    winnerValue,
    ownerIdempotencyKey,
  );
  if (!suppressionsAreCompatible(
    suppressions,
    winnerValue,
    minimumExpiry.getTime(),
    true,
    ownerIdempotencyKey,
  )) {
    throw erasureFailed();
...
export async function resolveContactStatusFromD1(database, contactRef) {
  ...
  try {
    ...
    if (
      ...
      || !CONTACT_STATES.has(value.state)
      || !RESTRICTION_STATES.has(value.restrictionState)
    ) throw erasureFailed();
    const erasedAt = value.erasedAt === '' ? '' : normalizeIsoTimestamp(value.erasedAt);
    if (
      (value.erasedAt !== '' && !erasedAt)
      || (value.state === 'erased' && (!erasedAt || value.restrictionState !== 'none'))
      || (value.state === 'new' && erasedAt)
    ) throw erasureFailed();
    return {
      found: true,
      contactStatus: value.state === 'erased' ? 'erased' : 'active',
...
export async function applyContactErasureToD1(database, input, overrides = {}) {
  ...
  const storedLead = selectedDataRecord(lead, ['id', 'erasedAt', 'idempotencyKey']);
  if (
    !storedLead
    || typeof storedLead.id !== 'string'
    || typeof storedLead.erasedAt !== 'string'
    || typeof storedLead.idempotencyKey !== 'string'
    || !UUID.test(storedLead.id)
    || normalizeUuid(storedLead.id) !== value.leadId
  ) throw erasureFailed();
  let refHash;
  let requestHash;
  let erasedIdempotency;
  try {
    ...
    if (storedLead.erasedAt) {
      if (!ERASED_IDEMPOTENCY.test(storedLead.idempotencyKey)) throw erasureFailed();
      erasedIdempotency = storedLead.idempotencyKey;
    } else {
      const keys = await deriveContactIntakeIdempotencyKeys(storedLead.idempotencyKey);
      if (!keys) throw erasureFailed();
      erasedIdempotency = keys.tombstoneKey;
    }
  } catch {
    throw erasureFailed();
  }
  ...
  let ownershipProof;
  try {
    ownershipProof = await sha256(JSON.stringify([requestHash, suppressionId, lifecycleId]));
  } catch {
    throw erasureFailed();
  }
  ...
  const statements = [
    prepareStatement(
      ...
      [erasedIdempotency, ownershipProof, now, now, value.leadId],
    ),
    prepareStatement(
      database,
      `UPDATE crm_opportunities SET
        ...
        WHERE contact_ref = ? AND EXISTS (
          ...
          WHERE id = ? AND idempotency_key = ? AND erased_at = ? AND request_hash = ?
        )`,
      [
        tombstoneRef,
        now,
    ...
    prepareStatement(
      database,
      `UPDATE contact_leads SET request_hash = ''
        WHERE id = ? AND idempotency_key = ? AND erased_at = ? AND request_hash = ?`,
      [value.leadId, erasedIdempotency, now, ownershipProof],
    ),
  ];
