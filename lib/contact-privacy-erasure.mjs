...
async function validateCommittedErasure(
  ...
  if (!suppressionsAreCompatible(suppressions, value, minimumExpiryMs, true)) {
    throw erasureFailed();
  }

  const sameRequest = lead.idempotencyKey === erasedIdempotency;
  const lifecycle = await findCommittedLifecycle(database, refHash);
  if (!lifecycleMatches(lifecycle, value, refHash, requestHash, sameRequest)) {
    throw erasureFailed();
  }
  return { sameRequest };
}
