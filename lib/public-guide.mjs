...
import { PUBLIC_FAQ_ITEMS, faqEntries, matchPublicFaq } from './public-faq.mjs';
...
const SOURCE_CATALOG = Object.freeze({
  ...
  faq: Object.freeze({ id: 'faq', title: 'Preguntas frecuentes', url: '/preguntas-frecuentes' }),
  ...
  evolution: Object.freeze({ id: 'evolution', title: 'Evolucion agentica', url: '/evolucion-agentica' }),
  home: Object.freeze({ id: 'home', title: 'Inicio y auditoria publica', url: '/' }),
  siteMap: Object.freeze({ id: 'siteMap', title: 'Mapa del sitio', url: '/mapa-del-sitio' }),
  openKnowledge: Object.freeze({ id: 'openKnowledge', title: 'Conocimiento abierto OKF', url: '/conocimiento-abierto' }),
  mcp: Object.freeze({ id: 'mcp', title: 'MCP read-only', url: '/mcp-readonly' }),
  dossier: Object.freeze({ id: 'dossier', title: 'Expediente privado', url: '/expediente' }),
  externalVerification: Object.freeze({ id: 'externalVerification', title: 'Verificacion externa', url: '/verificacion-externa' }),
  tokenizartCase: Object.freeze({ id: 'tokenizartCase', title: 'Caso Tokenizart', url: '/casos/tokenizart' }),
});
...
const SOURCE_ROUTES = Object.freeze({
  ...
  faq: ['faq'],
  evolution: ['evolution'], home: ['home'], siteMap: ['siteMap'], openKnowledge: ['openKnowledge'],
  mcp: ['mcp'], dossier: ['dossier'], externalVerification: ['externalVerification'], tokenizartCase: ['tokenizartCase'],
});

const SOURCE_TITLES = Object.freeze({
  en: { overview: 'Complete site map', methodology: 'AF-0 to AF-5 methodology', audit: 'Public read-only auditor', aeo: 'AEO and crawler policy', registry: 'Public Registry', intake: 'Preparation assistant', expediente: 'Private dossier', okf: 'Open knowledge OKF', cli: 'Read-only CLI', tokenizart: 'Tokenizart case', faq: 'Frequently asked questions', readiness: 'Capability manifest', security: 'Security policy', evolution: 'Agentic evolution', home: 'Home and public audit', siteMap: 'Site map', openKnowledge: 'Open knowledge OKF', mcp: 'Read-only MCP', dossier: 'Private dossier', externalVerification: 'External verification', tokenizartCase: 'Tokenizart case' },
  pt: { overview: 'Mapa completo do site', methodology: 'Metodologia AF-0 a AF-5', audit: 'Auditor público read-only', aeo: 'AEO e política de crawlers', registry: 'Registry público', intake: 'Assistente de preparação', expediente: 'Dossiê privado', okf: 'Conhecimento aberto OKF', cli: 'CLI read-only', tokenizart: 'Caso Tokenizart', faq: 'Perguntas frequentes', readiness: 'Manifesto de capacidades', security: 'Política de segurança', evolution: 'Evolução agêntica', home: 'Início e auditoria pública', siteMap: 'Mapa do site', openKnowledge: 'Conhecimento aberto OKF', mcp: 'MCP read-only', dossier: 'Dossiê privado', externalVerification: 'Verificação externa', tokenizartCase: 'Caso Tokenizart' },
});
...
const PUBLIC_FAQ_IDS = new Set(PUBLIC_FAQ_ITEMS.map((entry) => entry.id));

function faqIdFromTopic(topic) {
  return typeof topic === 'string' && topic.startsWith('faq:') ? topic.slice(4) : null;
}

function faqEntryFromTopic(topic, locale) {
  const id = faqIdFromTopic(topic);
  return id ? faqEntries(locale).find((entry) => entry.id === id) || null : null;
}

function safeContext(context) {
  const faqId = faqIdFromTopic(context?.topic);
  const topic = context && (TOPICS[context.topic] || PUBLIC_FAQ_IDS.has(faqId)) ? context.topic : null;
...
function makeFaqTurn(entry, mode, locale = 'es') {
  const answerMode = ['simple', 'standard', 'detailed'].includes(mode) ? mode : 'standard';
  const sourceIds = ['faq', ...entry.sources].filter((id, index, all) => all.indexOf(id) === index);
  return {
    contract: 'agent-friendly-web.public-guide-turn.v1',
    topic: `faq:${entry.id}`,
    mode: answerMode,
    blocked: false,
    answer: answerMode === 'detailed' ? entry.detailedAnswer : entry.shortAnswer,
    quick_replies: [],
    sources: sourceIds.map((id) => localizedSource(id, locale)),
    next_context: {
      topic: `faq:${entry.id}`,
      mode: answerMode,
      pending_follow_up: null,
...
export function respondToPublicGuide({ locale: requestedLocale = 'es', message = '', context = PUBLIC_GUIDE_INITIAL_CONTEXT } = {}) {
  ...
  if (secretPattern.test(rawInput)) {
  ...
  const currentFaq = faqEntryFromTopic(current.topic, locale);
  if (simplePattern.test(input) && currentFaq) return makeFaqTurn(currentFaq, 'simple', locale);
  if (detailPattern.test(input) && currentFaq) return makeFaqTurn(currentFaq, 'detailed', locale);

  const matchedFaq = matchPublicFaq(input, locale);
  const faqRefinesGeneralTopic = matchedFaq && ['automatic-progression', 'llms-txt'].includes(matchedFaq.id);
  if (faqRefinesGeneralTopic) {
    const requestedMode = simplePattern.test(input) ? 'simple' : detailPattern.test(input) ? 'detailed' : current.mode;
    return makeFaqTurn(matchedFaq, requestedMode, locale);
  }
