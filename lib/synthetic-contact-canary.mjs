...
function turnstileExpectations(secret) {
  if (secret === TURNSTILE_ALWAYS_PASS_TEST_SECRET) {
    return { action: '', hostname: '' };
  }
  return { action: TURNSTILE_ACTION, hostname: CANARY_HOST };
}
...
export function createSyntheticContactCanaryHandler(overrides = {}) {
  ...
  return async function handleSyntheticContactCanary(request, env = {}) {
    ...
    const turnstileContext = turnstileExpectations(env.AFW_SYNTHETIC_CONTACT_TURNSTILE_SECRET);
    let contact;
    try {
      contact = await processContact(syntheticIntake(validation.value), {
        ...
        verifyTurnstile: ({ token, idempotencyKey }) => verifyTurnstile({
          ...
          remoteIp,
          idempotencyKey,
          action: turnstileContext.action,
          hostname: turnstileContext.hostname,
        }),
